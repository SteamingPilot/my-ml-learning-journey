...
sc = StandardScaler()
X_train_featured = sc.fit_transform(X_train[:, -7:])
X_test_featured = sc.transform(X_test[:, -7:])

# We don't want to feature scale the categorical variables.
X_train = np.hstack([X_train[:, :-7], X_train_featured[:, :]])
X_test = np.hstack([X_test[:, :-7], X_test_featured[:, :]])

X_featured = sc.transform(X[:, -7:])
X = np.hstack([X[:, :-7], X_featured[:, :]])
